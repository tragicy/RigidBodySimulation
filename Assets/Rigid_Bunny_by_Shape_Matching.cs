...
public class Rigid_Bunny_by_Shape_Matching : MonoBehaviour
{
	...
	void Update()
    {
		...
		Matrix4x4 YCR = Matrix4x4.zero;
		for (int i = 0; i < Q.Length; i+=10)
		{
			YCR[0, 0] += (X[i] - c).x * Q[i][0];
			YCR[0, 1] += (X[i] - c).x * Q[i][1];
			YCR[0, 2] += (X[i] - c).x * Q[i][2];
			YCR[1, 0] += (X[i] - c).y * Q[i][0];
			YCR[1, 1] += (X[i] - c).y * Q[i][1];
			YCR[1, 2] += (X[i] - c).y * Q[i][2];
			YCR[2, 0] += (X[i] - c).z * Q[i][0];
			YCR[2, 1] += (X[i] - c).z * Q[i][1];
			YCR[2, 2] += (X[i] - c).z * Q[i][2];
		}
		YCR[3, 3] = 1;
		R = Get_Rotation(YCR * QQt);
